...
export default async function SchoolPage({ params }: SchoolPageProps) {
  ...
  const city = getCityName(school);
  const citySchools = getSchoolsByCitySlug(getCitySlug(city))
    ...
    <SeoPageShell>
      ...
      <section className="rounded-lg border border-slate-200 bg-white p-6 sm:p-8">
        <div className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
          <div>
            <Link className="text-sm font-medium text-blue-700 hover:text-blue-900" href={`/locations/${getCitySlug(city)}`}>
              {city} schools
            </Link>
      ...
      {citySchools.length ? (
        <section className="rounded-lg border border-slate-200 bg-white p-6">
          <h2 className="text-xl font-bold">Other schools in {city}</h2>
          <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {citySchools.map((item) => (
              <Link
